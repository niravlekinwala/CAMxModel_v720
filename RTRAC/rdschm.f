...
c                               parameters
c    04/05/21   --cemery--      Enable RTCMC to read scaling factor for
c                               photolysis reactions that point to core
c                               mech reactions
c
c-----------------------------------------------------------------------
c    Include files:
      ...
      do i = 1, nrxnrtc
         if( nrct(i) .GT. 0 ) then
            do j = 1, nrct(i)
               ...
               if ( .NOT. lhit ) then
                  ngasrtc = ngasrtc + 1
                  if( ngasrtc .GT. MXTRSP ) goto 7002
...
c
 ...
 100  read (iinp, '(A)', end=8001, err=8000) line
     ...
     &                         reacschm(isp), rsclschm(isp)
         ...
         if (istrln(spnmschm(isp)).gt.8) goto 8005
      endif
      goto 100
...
c
 8004 write(iout,'(/,2(/,A))')
     ...
     &                   ' parameters listed'
      call camxerr()
c
 8005 write(iout,'(//,a)') 'ERROR in RDSCSPC:'
      write(iout,'(2a)')   'Species: ',spnmschm(isp)
...
c
 ...
 101  read (iinp, '(A)', end=200, err=8000) line
      ...
      if( ijschm(nrxn) .LT. 0 ) then
         read(line, *) ijschm(nrxn), ijpntr(nrxn),jscale(nrxn)
...
c
     ...
     &      '          mechanism reactions are renumbered sequentially'
         if (npnt.eq.0) then
            write(idiag,'(a)')
     &      '    No photolysis rates point to core model mechanism reactions'
         else
            write(idiag,'(/,a,/,a)')
     &      '    Photolysis rates pointing to core model mechanism reactions',
     &      '    Rxn Pntr   Scaling'
           do i = 1,njschm
             if (ijpntr(i).ne.0)
     &         write(idiag,'(2x,2i5,e10.2)') ijschm(i),ijpntr(i),jscale(i)
           enddo
         endif
      endif
c
