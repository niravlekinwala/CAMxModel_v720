...
c     05/12/21   --cemery--     Moved Koa parameter to gas species list
c
c-----------------------------------------------------------------------
c    Include files:
c-----------------------------------------------------------------------
...
c     if (lparttn .AND. nrtaero.lt.nrtgas) then
c        write(iout,'(A)') 'RTRAC gas-aerosol partitioning is selected.'
c        write(iout,'(A,A)') 'You must list at least the same number',
c    &                 ' of aerosol tracers as gas tracers.'
c        write(iout,'(A,A)') 'NOTE: gas tracer 1 will partition to',
c    &                       ' aerosol tracer 1, etc.'
c        write(iout,'(A)') 'CAMx is stopping.'
c        call camxerr()
c     endif
...
c
     ...
     &                       ' record in RTRAC Chemistry file.'
          read(iorchm,'(A)',ERR=7000,END=7001) line     
cae       write(idiag,'(A)',ERR=7009) line(:istrln(line))
          read(iorchm,'(A)',ERR=7000,END=7001) line     
cae       write(idiag,'(A)',ERR=7009) line(:istrln(line))
          do i = 1,nrtgas
            read(iorchm,'(A)',ERR=7000,END=7001) line     
...
c
            ...
            lsagas(i) = .TRUE.
            loutsa(i) = .TRUE.
            read(line,'(27X,7F10.0)',ERR=7005) rtlbnd(i), 
     &        rthlaw(i), rttfact(i), rtmolwt(i), rtreact(i), rtscale(i),
     &        eqkoa(i)
            rtdrate(i) = sqrt(rtmolwt(i)/18.)
            sa_mole_weight(i) = rtmolwt(i)
          enddo
c
c  --- echo the data to the diag file ----
c
          write(idiag,'(A)') 'RTRAC gas species values'
          write(idiag,'(3A)') '     Name      P/S   Pname LowBnd (ppm)',
     &                        '   H (M/atm)   Tfact (K)  MW (g/mol)',
     &                        '  Reactivity      Rscale         Koa'
ciii  aaaaaaaaaaAAAAAAaaaaaa  -e.eeeE-ee  -e.eeeE-ee-fffffff.fff-fffffff.fff-fffffff.fff-fffffff.fff

          do i = 1,nrtgas
             write(idiag,'(I3,2X,A10,2A6,2E12.3,4F12.3,E12.3)',ERR=7009) i,
     &              ptname(i),flag(i),sectmp(i),rtlbnd(i),rthlaw(i), 
     &              rttfact(i),rtmolwt(i),rtreact(i),rtscale(i),eqkoa(i)
             if (eqkoa(i).lt.0.) then
               write(iout,'(//,A)') 'ERROR in RDCHMRT:'
               write(iout,'(3A)') 'Koa for ',ptname(i)(:istrln(ptname(i))),
     &                            ' is <0! --- Stopping'
               call camxerr() 
             endif
             if (istrln(ptname(i)).gt.8) goto 7012
          enddo
      endif
c
...
c
     ...
     &                       ' in RTRAC Chemistry file.'
         ...
         do i = 1,nrtgas
           ...
           read(line,'(15X,4F10.0)',ERR=7005)
     &                     khydro(i),kleach(i),kpen(i),kphot(i)
           write(idiag,'(I3,2X,A10,4E10.3)',ERR=7009) i,
     &           ptname(i),khydro(i),kleach(i),kpen(i),kphot(i)
           if (khydro(i).lt.0. .or. kleach(i).lt.0.
     &         .or. kpen(i).lt.0. .or. kphot(i).lt.0.) then
             write(iout,'(//,A)') 'ERROR in RDCHMRT:'
             write(iout,'(3A)') 'Rates for ',ptname(i)(:istrln(ptname(i))),
     &                         ' are <0!'
...
c
            ...
            lsagas(i) = .FALSE.
            loutsa(i) = .TRUE.
            sa_mole_weight(i) = 1.0
            read(line,'(15X,4F10.0)',ERR=7005) rtlbnd(i), 
...
c
         do i = 1,nrtgas
            write(idiag,'(A10,I10,F10.4)',ERR=7009) ptname(i),
     &                                           jnum(i), rtjfact(i)
         enddo
      endif
c
...
c
     ...
     &        'Diagnostic info for checking RTRAC rate expressions'
        write(idiag,'(/,a,/,a)')
     &        'Rates at 298 K and 1013 mbar in cm3 molec-1 s-1',
     ...
     &                                           rkoh, rko3, rkno3
        enddo
        write(idiag,'(/,a,/,a)')
     &        'Rates at 273 K and 1013 mbar in cm3 molec-1 s-1',
     &        '      Species        OH rate     O3 rate    NO3 rate'
        adj = 298./273.
        do i = 1,nrtgas
...
c
 7011 continue
     ...
     &                  ' other RTRAC tracers if IRON PiG',' is invoked.'
      call camxerr()
c
 7012 write(iout,'(//,a)') 'ERROR in RDCHMRT:'
      write(iout,'(2a)')   'Species: ',ptname(i)
