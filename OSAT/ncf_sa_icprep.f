      subroutine ncf_sa_icprep(begtim,begdate,endtim,enddate)
      use filunit
      use tracer
      implicit none
c
...
c-----------------------------------------------------------------------
c
c     NCF_SA_ICPREP reads and checks the SA initial condition file.
...
c
      ...
      character*10          icfiltyp, name_in, this_var
      character*3           first_class
      integer               this_dimid, ierr, this_varid, itmp, nvars_in, l
c
      character*10,     allocatable :: icspec(:)
c
c-----------------------------------------------------------------------
c    Data statments:
c-----------------------------------------------------------------------
c
      data icfiltyp /'INITIAL   '/
c
c-----------------------------------------------------------------------
c    Entry point:
c-----------------------------------------------------------------------
...
c
      ...
      if( name_in(:8) .NE. icfiltyp(:8) ) goto 7000
c
...
c
      varlist = ' '
      this_var = 'VAR-LIST'
      ierr = nf_get_att_text(ioric, NF_GLOBAL, this_var, varlist)
      if( ierr .NE. NF_NOERR ) goto 7002
      num_ioric = 0
      do l=1,nvars_in
         if( varlist(1+(l-1)*16:16+(l-1)*16) .NE. '                ' ) 
     &                                         num_ioric = num_ioric + 1
      enddo
      allocate( spc_ioric(num_ioric) )
      first_class = varlist(1:3)
      do l=1,num_ioric
         spc_ioric(l) = varlist(1+(l-1)*16:10+(l-1)*16)
         if( spc_ioric(l)(1:3) .EQ. first_class ) 
     &                                         ncls_ioric = ncls_ioric + 1
      enddo
c
      goto 9999
c
...
c
 7000 continue
      ...
      write(iout,'(2A)') 'Looking for type: ',icfiltyp(:istrln(icfiltyp))
