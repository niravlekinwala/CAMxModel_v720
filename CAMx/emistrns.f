...
c       07/10/19    Added stratospheric O3 adjustment
...
c======================== DDM Begin =======================
c
c-----Optionally make stratospheric O3 adjustment
c
          if (lstrato3 .and. ltopcon) then
            call strato3(mxp,myp,mzp,ia,iz,ja,jz,ibcon,igrd,nspec,
     &                   idfin(iptr2d(igrd)),height(iptr3d(igrd)),
     &                   tempk(iptr3d(igrd)),press(iptr3d(igrd)),
     &                   conc(iptr4d(igrd)),topo(iptr2d(igrd)),
     &                   ctop(iptrem(igrd)))
          endif
c
...
c
          call diffus(mxp,myp,mzp,i0,j0,ia,iz,ja,jz,ibcon,.FALSE.,.FALSE.,
...
c
     ...
     &                  .FALSE.,.TRUE.,.TRUE.,igrd,ncol(igrd),nrow(igrd),
     ...
     &                  '  SA z diffus ......','  SA x/y diff ......')
               elseif( tectyp .EQ. RTRAC .OR. tectyp .EQ. RTCMC ) then
                   call diffus(mxp,myp,mzp,i0,j0,ia,iz,ja,jz,ibcon,
     &                  .TRUE.,.TRUE.,.TRUE.,igrd,ncol(igrd),nrow(igrd),
     ...
     &                  '  SA z diffus ......','  SA x/y diff ......')
                   ...
                   call diffus(mxp,myp,mzp,i0,j0,ia,iz,ja,jz,ibcon,
     &                  .FALSE.,.TRUE.,.FALSE.,igrd,ncol(igrd),nrow(igrd),
     &                  nlay(igrd),0,ntotsp,notimespc,1,1,deltat(igrd),
     &                  deltax(1,igrd),
     &                  deltay(igrd),idfin(iptr2d(igrd)),ptvdep,
     &                  rkx(iptr3d(igrd)),rky(iptr3d(igrd)),
